...
export default function ARView({ coin, onBack }) {
  ...
  useEffect(() => {
    ...
    if (isIOS()) {
      if (iosPermissionGranted) window.addEventListener('deviceorientation', handleOrientation, true);
    } else {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (modelRef.current && currentLocation && userHeading !== null) {
        ...
        let angleDiffCalc = Math.abs(((bearingToCoin - userHeading) + 360) % 360);
        if (angleDiffCalc > 180) angleDiffCalc = 360 - angleDiffCalc;
        setAngleDiff(angleDiffCalc);

        // Target position
        const targetPos = latLngToPosition(currentLocation, coin);
        const prevPos = modelRef.current.position;

        // Dynamic smoothing factor based on distance to target (fast/slow movement)
        ...
        let headingFactor = Math.min(0.3, 0.05 + Math.abs(userHeading - prevHeading)/180);
        ...
        // Visibility based on angle & distance
        modelRef.current.visible = angleDiffCalc <= 90 && distance <= 100;
  ...
  }, [coin, userHeading, iosPermissionGranted]);
  ...
  const handleCollect = () => {
    if (canCollectRef.current) {
      ...
      alert(`🚫 Move closer and face the coin!`);
    }
  };
    ...
    <div ref={containerRef} style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      {isIOS() && !iosPermissionGranted && (
        <button
          ...
          style={{ position: 'absolute', top: 110, left: 20, zIndex: 20, padding: 8, backgroundColor: '#007bff', color: 'white', borderRadius: 6, border: 'none', cursor: 'pointer' }}
      ...
      <button
        ...
        disabled={!canCollect}
        style={{ position: 'absolute', bottom: 40, left: '50%', transform: 'translateX(-50%)', padding: 12, fontSize: 18, fontWeight: 'bold', borderRadius: 8, border: 'none', cursor: canCollect ? 'pointer' : 'not-allowed', backgroundColor: canCollect ? '#22c55e' : '#94a3b8', color: 'white', zIndex: 10 }}
      >
